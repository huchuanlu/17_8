function [test_bag_label, test_inst_label, test_bag_pre, test_ins_pre] = Inst_KISVM_prediction(alpha, y, y_set, beta, inx_train, K_test, inx_test)

%Inst_KISVM_prediction is a convex method for locating Region Of Interest(ROI) with multi-instance learning 
%
%N.B.: Inst_KISVM_prediction employs a new Matlab software (libsvm-mat-2.88-MI-SVM)
%
%    Syntax
%
%       function [test_bag_label, test_inst_label, test_bag_pre,test_ins_pre] = Inst_KISVM_prediction(alpha, y, y_set, beta, inx_train, K_test, inx_test)
...
for i = 1:n_test
   ...
   test_inst_label{i} = sign(y2(inx_test(i):inx_test(i+1)-1));
    % test_inst_label{i} = (y2(inx_test(i):inx_test(i+1)-1));
